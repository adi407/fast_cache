...
An in-memory, vertically sharded, off-heap key-value store for AI workloads — embedded in Java via one
annotation, or driven from Python via one decorator with no infrastructure to install.
...
Both snippets are complete. There is no server to start, no connection string, no `CacheManager` bean, no
Docker container, and no Redis. Open `http://127.0.0.1:8081/dashboard` to watch what it is saving you.
...
cp fastcache-engine/target/fastcache-engine.jar python/fastcache/_bin/
...
`core/RefreshCoordinator.java`, `core/CacheEntry.java`, `python/fastcache/singleflight.py`
...
`net/OrphanWatchdog.java`, `python/fastcache/client.py`
...
`python/fastcache/bootstrap.py`, `net/SidecarMain.java`
